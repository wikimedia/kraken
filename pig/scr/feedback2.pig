...
loglines = LOAD '/user/diederik/pagecounts-raw/2012/2012-09/pagecounts-20120930-0*' USING org.wikimedia.analytics.kraken.loaders.PigStorageWithInputPath(' ','-tagsource') AS (path:chararray, project_code:chararray, title:chararray, views:int, response_size:long);
...
titles  = FOREACH feedback GENERATE page_title, CONCAT('Talk:',page_title) AS 
talk_title:chararray, CONCAT('Special:ArticleFeedbackv5/', page_title) AS 
feedback_title:chararray;
...
feedback = GROUP joined BY (timestamp, title, page_id);
