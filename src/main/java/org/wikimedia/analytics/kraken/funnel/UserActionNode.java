...
class UserActionNode extends Node{
	 ...
	 */
	public UserActionNode(Map<String, String> nodeDefinition, JsonObject json)  {
		/**
        ...
        for (Map.Entry<String, String> entry : nodeDefinition.entrySet()) {
            String componentType = entry.getKey().toUpperCase();
            String componentValue = entry.getValue();
            JsonElement jsonFunnelParam = json.get(componentValue);
            ComponentType type = ComponentType.valueOf(componentType);
            System.out.println("type: " + type.toString() + " value: " + jsonFunnelParam.getAsString());
            switch (type) {
                case PROJECT:
                    componentValues.putAll(splitProject(jsonFunnelParam.getAsString()));
                default:
                    componentValues.put(type, jsonFunnelParam.getAsString());
            }
        }
	}
